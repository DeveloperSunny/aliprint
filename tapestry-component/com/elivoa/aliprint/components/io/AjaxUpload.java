...
import org.iminer.t5.upload.AjaxUploadDecoder;
...
@Import(stylesheet = { "_/ajaxupload.css" }, library = { "_/ajaxupload.js" })
public class AjaxUpload extends AbstractField {

	public static final String FILE_PARAMETER = "qqfile";

	private static final String STYLE_TO_HIDE_INPUT_TEXT = "display:none;"
			+ "color:transparent;background:transparent;border:0;height:1px;width:1px;";

	@Parameter(required = false, autoconnect = true, principal = true)
	...
	void afterRender(final MarkupWriter writer) {
		writer.end();
		writer.element("span", "id", getWrapperClientId(), "style",
				"display:inline-block; position: relative; width:99%");
		writer.end();
	}
	...
	JSONObject fillArguments() {
		...
		spec.put("redirectLink", this.redirectLink);
		...
		spec.put("container_zone", container_zone);
		return spec;
	}
	...
	/**
	 * Converts the current list of uploaded files to a JSON object containing a
	 * json array with each element containing the name of the file and a unique
	 * key for identification. The unique key is index of the uploaded file in
	...
	Object onUpload() {
		...
		if (uploadedFile.getSize() > maxSize) {
			return createFailureResponse("The file exceeds the limit size 10M.");
		}
	...
	private UploadedFile createUploadedFileFromMultipartForm() {
		if (logger.isInfoEnabled()) {
			logger.info("Create uploaded file...");
		}

		UploadedFile file = multipartDecoder.getFileUpload(FILE_PARAMETER);

		if (logger.isInfoEnabled()) {
			logger.info("Create uploaded file... done");
		}
		return file;
	}

	private UploadedFile createUploadedFileFromRequestInputStream() {
		UploadedFile file = ajaxDecoder.getFileUpload();
		return file;
	}
	...
	void onRemoveUpload(@RequestParameter("serverIndex") int serverIndex) {
		...
		// the client side and if the uploaded file is removed we cleanup and
		// set
		// the element at that index to null. As the 'value' may contain null,
		// we
	...
	protected void processSubmission(String elementName) {
		...
		if (logger.isInfoEnabled()) {
			logger.info("Process Submisstion");
		}
		if (value != null) {
